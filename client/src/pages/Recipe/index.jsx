...
export function Recipe() {
  ...
  async function getRecipeDetails() {
    if (!params.id) return;
    setIsLoading(true);
    const response = await axios.get(`http://localhost:3000/api/recipesApi/allRecipes/${params.id}`);
      ...
      {isLoading ? (
        ...
        <>
          ...
          {recipe.instructions && (
            <>
              <h3>Instructions</h3>
              <h4 key="instructions" dangerouslySetInnerHTML={{ __html: recipe.instructions }} />
            </>
          )}
        </>
      )}
